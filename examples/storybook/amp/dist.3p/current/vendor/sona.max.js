(function() {
  ...
  // ads/vendors/sona.js
  function sona(global, data) {
    validateData(data, ["config"], ["responsive"]);
    var dataConfig = data["config"];
    var adConfig = parseJson(dataConfig);
    var configScript = global.document.createElement("SCRIPT");
    var config = global.document.createTextNode("(sona = window.sona || " + JSON.stringify(adConfig) + ")");
    configScript.appendChild(config);
    var slot = global.document.getElementById("c");
    var ad = global.document.createElement("SONA-WIDGET");
    ad.setAttribute("auto-responsive", "");
    ad.className = "ad-tag";
    slot.appendChild(ad);
    slot.appendChild(configScript);
    var scriptUrl = "https://cdn.sonaserve.com/v1.1/dist.js";
...
//# sourceMappingURL=sona.max.js.map
